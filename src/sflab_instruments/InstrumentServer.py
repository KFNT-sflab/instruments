...
class InstrumentClientListener:
    ...
    def start(self):
        ...
        print(f"Strting server on port {self.port}.")
    ...
    def accept(self):
        ...
        c = InstrumentClientHandler(
            conn, self.end_event, name=handler_name, finished=self.finished_handlers, visa_instruments=self.instruments
        )
...
def main():
    log.basicConfig(
        filename="instrument_server_log.txt", format="%(asctime)s %(levelname)s:%(message)s", level=log.INFO
    )
    try:
        with VISAInstruments() as instruments, InstrumentClientListener(instruments, address="") as server:
            server.start()
            server.loop()
